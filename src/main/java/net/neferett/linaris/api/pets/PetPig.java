...
public class PetPig extends Pet {
	...
	public PetPig(Player player) {
		super("Cochon", "§bCochon", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.PIG);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.PIG);
