...
public class PetZombie extends Pet {
	...
	public PetZombie(Player player) {
		super("Zombie", "§bZombie", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.ZOMBIE);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.ZOMBIE);
