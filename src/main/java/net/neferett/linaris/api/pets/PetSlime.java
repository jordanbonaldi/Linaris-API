...
public class PetSlime extends Pet {
	...
	public PetSlime(Player player) {
		super("Slime", "§bSlime", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.SLIME);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.SLIME);
