...
public class PetLapin extends Pet {
	...
	public PetLapin(Player player) {
		super("Lapin", "§bLapin", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.RABBIT);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.RABBIT);
