...
public class PetMagMaCube extends Pet {
	...
	public PetMagMaCube(Player player) {
		super("MagmaCube", "§bMagmaCube", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.MAGMA_CUBE);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.MAGMA_CUBE);
