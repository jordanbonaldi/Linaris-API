...
public class PetSilverFish extends Pet {
	...
	public PetSilverFish(Player player) {
		super("SilverFish", "§bSilver Fish", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.SILVERFISH);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.SILVERFISH);
