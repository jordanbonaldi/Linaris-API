...
public class PetCreeper extends Pet {
	...
	public PetCreeper(Player player) {
		super("Creeper", "§bCreeper", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.CREEPER);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.CREEPER);
