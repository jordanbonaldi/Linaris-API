...
public class PetWolf extends Pet {
	...
	public PetWolf(Player player) {
		super("Chien", "§bChien", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.WOLF);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.WOLF);
