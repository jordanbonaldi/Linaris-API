...
public class PetChicken extends Pet {
	...
	public PetChicken(Player player) {
		super("Poulet", "§bPoulet", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.CHICKEN);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.CHICKEN);
