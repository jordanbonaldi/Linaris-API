...
public class PetWither extends Pet {
	...
	public PetWither(Player player) {
		super("Wither", "§bWither", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.WITHER);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.WITHER);
