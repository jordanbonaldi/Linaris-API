...
public class PetSheep extends Pet {
	...
	public PetSheep(Player player) {
		super("Mouton", "§bMouton", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.SHEEP);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.SHEEP);
