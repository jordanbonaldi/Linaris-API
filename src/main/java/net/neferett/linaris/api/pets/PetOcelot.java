...
public class PetOcelot extends Pet {
	...
	public PetOcelot(Player player) {
		super("Chat", "§bChat", player);
	...
	public void spawnPacket(Player target) {
		...
		spawnPacket.setType(EntityType.OCELOT);


		Location loc = getShoulder();
		
		WrappedDataWatcher watcher = WrappedUtils.getDefaultWatcher(loc.getWorld(), EntityType.OCELOT);
