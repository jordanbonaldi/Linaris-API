...
public class ReflectionManager {
	...
	public enum LibVersion {
		...
		static {
			// String mcVersion = Bukkit.getVersion().split("MC:
			// ")[1].replace(")", "");
			currentVersion = V1_8;
		}
	...
	/**
	 * Map of Forge fully qualified class names to a map from mc-dev field names
	 * to Forge field names.
	 */
	private static Map<String, Map<String, String>> ForgeFieldMappings;
	/**
	 * Map of Forge fully qualified class names to a map from mc-dev method
	 * names to a map from method signatures to Forge method names.
	 */
	 ...
	 *
	 * In addition to the implicit license granted to libraryaddict to
	 * redistribuite the code, the code is also licensed to the public under the
	 * BSD 2-clause license.
	 *
	 * The publicly licensed version may be viewed here:
	 * https://gist.github.com/riking/2f330f831c30e2276df7
	 */
	static {
		...
		primitiveTypes = ImmutableMap.<Class<?>, String>builder().put(boolean.class, "Z").put(byte.class, "B")
				.put(char.class, "C").put(short.class, "S").put(int.class, "I").put(long.class, "J")
				.put(float.class, "F").put(double.class, "D").put(void.class, "V").build();
		...
		if (isForge) {
			...
			try {
				final InputStream stream = Class.forName("net.minecraftforge.common.MinecraftForge").getClassLoader()
				...
				final Pattern classPattern = Pattern
						.compile("^CL: net/minecraft/server/([" + nameseg_class + "]+) ([" + fqn_class + "]+)$");
				// 1: cb-simpleName
				// 2: cb-fieldName
				// 3: forge-fullName (Needs dir2fqn())
				// 4: forge-fieldName
				final Pattern fieldPattern = Pattern.compile("^FD: net/minecraft/server/([" + nameseg_class + "]+)/(["
						+ nameseg_class + "]+) ([" + fqn_class + "]+)/([" + nameseg_class + "]+)$");
				// 1: cb-simpleName
				// 2: cb-methodName
				// 3: cb-signature-args
				// 4: cb-signature-ret
				...
				final Pattern methodPattern = Pattern
						.compile("^MD: net/minecraft/server/([" + fqn_class + "]+)/([" + nameseg_class + "]+) \\(([;\\["
				...
				while ((line = reader.readLine()) != null) {
					...
					final Matcher fieldMatcher = fieldPattern.matcher(line);
					...
					if (methodMatcher.matches()) {
						...
						Map<String, Map<String, String>> middleMap = ForgeMethodMappings
								.get(dir2fqn(methodMatcher.group(5)));
						if (middleMap == null) {
				...
				System.out
						.println("[LibsDisguises] Loaded " + ForgeMethodMappings.size() + " Cauldron method mappings");
			} catch (ClassNotFoundException | IOException e) {
				e.printStackTrace(System.out);
				System.err.println(
						"Warning: Running on Cauldron server, but couldn't load mappings file. LibsDisguises will likely crash!");
			}
		}
	}

	static {
		for (final Method method : getNmsClass("EntityLiving").getDeclaredMethods())
			try {
				if (method.getReturnType() == float.class && Modifier.isProtected(method.getModifiers())
						&& method.getParameterTypes().length == 0) {
					final Object entity = createEntityInstance("Cow");
					method.setAccessible(true);
					final float value = (Float) method.invoke(entity);
					if (value == 0.4F) {
	...
	public static Object createEntityInstance(String entityName) {
		try {
			final Class<?> entityClass = getNmsClass("Entity" + entityName);
			Object entityObject;
			final Object world = getWorld(Bukkit.getWorlds().get(0));
			switch (entityName) {
			case "Player":
				final Object minecraftServer = getNmsMethod("MinecraftServer", "getServer").invoke(null);
				final Object playerinteractmanager = getNmsClass("PlayerInteractManager")
						.getConstructor(getNmsClass("World")).newInstance(world);
				final WrappedGameProfile gameProfile = getGameProfile(null, "LibsDisguises");
				entityObject = entityClass
						.getConstructor(getNmsClass("MinecraftServer"), getNmsClass("WorldServer"),
	...
	public static FakeBoundingBox getBoundingBox(Entity entity) {
		try {
			final Object boundingBox = getNmsMethod("Entity", "getBoundingBox").invoke(getNmsEntity(entity));
			double x = 0, y = 0, z = 0;
			int stage = 0;
			for (final Field field : boundingBox.getClass().getFields())
	...
	public static Object getEntityTrackerEntry(Entity target) throws Exception {
		final Object world = getWorld(target.getWorld());
		final Object tracker = trackerField.get(world);
		final Object trackedEntities = entitiesField.get(tracker);
		return ihmGet.invoke(trackedEntities, target.getEntityId());
	}

	public static String getEnumArt(Art art) {
		try {
			final Object enumArt = getCraftClass("CraftArt").getMethod("BukkitToNotch", Art.class).invoke(null, art);
			for (final Field field : enumArt.getClass().getFields())
	...
	public static WrappedGameProfile getGameProfileWithThisSkin(UUID uuid, String playerName,
		...
		try {
			final WrappedGameProfile gameProfile = new WrappedGameProfile(uuid != null ? uuid : UUID.randomUUID(),
					playerName);
	...
	public static Class getNmsClass(String className) {
		if (isForge) {
			final String forgeName = ForgeClassMappings.get(className);
			if (forgeName != null)
				try {
	...
	public static Method getNmsMethod(Class<?> clazz, String methodName, Class<?>... parameters) {
		if (isForge)
			try {
				final Map<String, String> innerMap = ForgeMethodMappings.get(clazz.getName()).get(methodName);
				final StringBuilder sb = new StringBuilder();
				for (final Class<?> cl : parameters)
	...
	public static Object getPlayerInfoData(Object playerInfoPacket, WrappedGameProfile gameProfile) {
		try {
			final Object playerListName = getNmsClass("ChatComponentText").getConstructor(String.class)
					.newInstance(gameProfile.getName());
			return getNmsClass("PacketPlayOutPlayerInfo$PlayerInfoData")
					.getConstructor(getNmsClass("PacketPlayOutPlayerInfo"), gameProfile.getHandleType(), int.class,
	...
	public static float[] getSize(Entity entity) {
		try {
			final float length = getNmsField("Entity", "length").getFloat(getNmsEntity(entity));
			final float width = getNmsField("Entity", "width").getFloat(getNmsEntity(entity));
			final float height = (Float) getNmsMethod("Entity", "getHeadHeight").invoke(getNmsEntity(entity));
	...
	public static WrappedGameProfile getSkullBlob(WrappedGameProfile gameProfile) {
		try {
			final Object minecraftServer = getNmsMethod("MinecraftServer", "getServer").invoke(null);
			for (final Method method : getNmsClass("MinecraftServer").getMethods())
				if (method.getReturnType().getSimpleName().equals("MinecraftSessionService")) {
					final Object session = method.invoke(minecraftServer);
		...
		try {
			...
			for (final Method method : getNmsClass("MinecraftServer").getMethods())
				if (method.getReturnType().getSimpleName().equals("GameProfileRepository")) {
					final Object profileRepo = method.invoke(minecraftServer);
					final Object agent = Class.forName("com.mojang.authlib.Agent").getField("MINECRAFT").get(null);
					final LibsProfileLookupCaller callback = new LibsProfileLookupCaller();
	...
	public static void setAllowSleep(Player player) {
		try {
			/**
			 * Object nmsEntity = getNmsEntity(player); Object connection =
			 * getNmsField(nmsEntity.getClass(),
			 * "playerConnection").get(nmsEntity); Field check =
			 * getNmsField(connection.getClass(), "checkMovement");
	...
	public static void setBoundingBox(Entity entity, FakeBoundingBox newBox) {
		try {
			...
			final Object boundingBox = boundingBoxConstructor.newInstance(loc.getX() - newBox.getX(),
