...
import net.neferett.linaris.disguises.disguisetypes.watchers.SheepWatcher;
import net.neferett.linaris.disguises.disguisetypes.watchers.WolfWatcher;
import net.neferett.linaris.disguises.utilities.DisguiseSound.SoundType;
...
public class PacketsManager {
	...
	public static void addPacketListeners() {
		// Add a client listener to cancel them interacting with uninteractable
		// disguised entitys.
		// You ain't supposed to be allowed to 'interact' with a item that
		// cannot be clicked.
		...
		clientInteractEntityListener = new PacketAdapter(libsDisguises, ListenerPriority.NORMAL,
			...
			public void onPacketReceiving(PacketEvent event) {
				...
				try {
					...
					// LoggerUtils.info(observer.getName());

					if (GhostManager.isGhost(observer.getName()))
						// LoggerUtils.info("Disguise Cancel Ghost !!");
						return;

					final StructureModifier<Entity> entityModifer = event.getPacket()
							.getEntityModifier(observer.getWorld());
					final Entity entity = entityModifer.read(0);
					if (entity instanceof ExperienceOrb || entity instanceof Item || entity instanceof Arrow
							|| entity == observer)
						event.setCancelled(true);
					final ItemStack item = observer.getItemInHand();
					if (item != null && item.getType() == Material.INK_SACK) {
						...
						if (disguise != null && (disguise.getType() == DisguiseType.SHEEP
								|| disguise.getType() == DisguiseType.WOLF)) {
							final AnimalColor color = AnimalColor.getColor(item.getDurability());
							if (disguise.getType() == DisguiseType.SHEEP) {
								final SheepWatcher watcher = (SheepWatcher) disguise.getWatcher();
								watcher.setColor(DisguiseConfig.isSheepDyeable() ? color : watcher.getColor());
							} else {
								final WolfWatcher watcher = (WolfWatcher) disguise.getWatcher();
								watcher.setCollarColor(
										DisguiseConfig.isWolfDyeable() ? color : watcher.getCollarColor());
							}
						}
					}
	...
	public static PacketContainer[][] constructSpawnPackets(final Player player, Disguise disguise,
			Entity disguisedEntity) {
		if (disguise.getEntity() == null)
			disguise.setEntity(disguisedEntity);
		final Object nmsEntity = ReflectionManager.getNmsEntity(disguisedEntity);
		final ArrayList<PacketContainer> packets = new ArrayList<>();
		...
		if (DisguiseConfig.isEquipmentPacketsEnabled())
			for (int nmsSlot = 0; nmsSlot < 5; nmsSlot++) {
				...
				if (armorSlot < 0)
					armorSlot = 4;
				final ItemStack itemstack = disguise.getWatcher().getItemStack(armorSlot);
		...
		if (DisguiseConfig.isMiscDisguisesForLivingEnabled())
			if (disguise.getWatcher() instanceof LivingWatcher) {
				...
				final Builder builder = WrappedAttribute.newBuilder().attributeKey("generic.maxHealth");
		...
		final Location loc = disguisedEntity.getLocation().clone().add(0, getYModifier(disguisedEntity, disguise), 0);
		...
		if (disguise.getType() == DisguiseType.EXPERIENCE_ORB) {
			...
			final StructureModifier<Object> mods = spawnPackets[0].getModifier();
			mods.write(0, disguisedEntity.getEntityId());
			...
			final int id = ((MiscDisguise) disguise).getData();
			...
			final StructureModifier<String> stringMods = spawnPackets[0].getStrings();
			...
			if (stringMods.size() > 0)
				...
				final PlayerDisguise playerDisguise = (PlayerDisguise) disguise;
				final String name = playerDisguise.getSkin() != null ? playerDisguise.getSkin()
						: playerDisguise.getName();
			...
			final StructureModifier<Integer> intMods = spawnPackets[0].getIntegers();
			...
			final StructureModifier<Byte> byteMods = spawnPackets[0].getBytes();
			byteMods.write(0, yaw);
			byteMods.write(1, pitch);
			spawnPackets[0].getDataWatcherModifier().write(0, createDataWatcher(player,
					WrappedDataWatcher.getEntityWatcher(disguisedEntity), disguise.getWatcher()));

			if (DisguiseConfig.isBedPacketsEnabled() && ((PlayerWatcher) disguise.getWatcher()).isSleeping()) {
				final PacketContainer[] newPackets = new PacketContainer[spawnPackets.length + 1];
				System.arraycopy(spawnPackets, 1, newPackets, 2, spawnPackets.length - 1);
				newPackets[0] = spawnPackets[0];
				spawnPackets = newPackets;
				final PacketContainer[] bedPackets = DisguiseUtilities.getBedPackets(player,
						loc.clone().subtract(0, PacketsManager.getYModifier(disguisedEntity, disguise), 0),
						player.getLocation(), ((PlayerDisguise) disguise));
				for (int i = 0; i < 2; i++)
					spawnPackets[i + 1] = bedPackets[i];
			}

			final ArrayList<PacketContainer> newPackets = new ArrayList<>();
			newPackets.add(null);
			for (final PacketContainer spawnPacket : spawnPackets)
				if (spawnPacket != null)
			...
			final PlayerDisguise playerDisguise = (PlayerDisguise) disguise;
			playerList.add(
					ReflectionManager.getPlayerInfoData(spawnPackets[0].getHandle(), playerDisguise.getGameProfile()));
			spawnPackets[0].getModifier().write(1, playerList);
			final PacketContainer delayedPacket = spawnPackets[0].shallowClone();
			...
			final DisguiseValues values = DisguiseValues.getDisguiseValues(disguise.getType());
			final Vector vec = disguisedEntity.getVelocity();
			spawnPackets[0] = new PacketContainer(PacketType.Play.Server.SPAWN_ENTITY_LIVING);
			final StructureModifier<Object> mods = spawnPackets[0].getModifier();
			mods.write(0, disguisedEntity.getEntityId());
			mods.write(1, disguise.getType().getTypeId());
			final double d1 = 3.9D;
			double d2 = vec.getX();
			double d3 = vec.getY();
			double d4 = vec.getZ();
			if (d2 < -d1)
			...
			final int id = disguise.getType().getEntityId();
			int data = ((MiscDisguise) disguise).getData();
			if (disguise.getType() == DisguiseType.FALLING_BLOCK)
				data = (((MiscDisguise) disguise).getId() | data << 16);
			else if (disguise.getType() == DisguiseType.FISHING_HOOK && data == 0)
				// If the MiscDisguise data isn't set. Then no entity id was
				// provided, so default to the owners entity id
		...
		if (spawnPackets[1] == null || disguise.isPlayerDisguise()) {
			...
			final StructureModifier<Object> mods = spawnPackets[entry].getModifier();
			mods.write(0, disguisedEntity.getEntityId());
			mods.write(1, yaw);
		}
	 ...
	 */
	private static WrappedDataWatcher createDataWatcher(Player player, WrappedDataWatcher watcher,
			FlagWatcher flagWatcher) {
		final WrappedDataWatcher newWatcher = new WrappedDataWatcher();
		try {
			...
			for (final WrappedWatchableObject watchableObject : list)
	 ...
	 */
	public static void init(BukkitAPI plugin) {
		libsDisguises = plugin;
		soundsListener = new PacketAdapter(libsDisguises, ListenerPriority.NORMAL,
				PacketType.Play.Server.NAMED_SOUND_EFFECT, PacketType.Play.Server.ENTITY_STATUS) {
			...
			public void onPacketSending(PacketEvent event) {
				...
				if (GhostManager.isGhost(observer))
					return;

				if (event.getPacketType() == PacketType.Play.Server.NAMED_SOUND_EFFECT) {
					if (event.isAsync())
						return;
					final String soundName = (String) mods.read(0);
					SoundType soundType = null;
					final Location soundLoc = new Location(observer.getWorld(), ((Integer) mods.read(1)) / 8D,
							((Integer) mods.read(2)) / 8D, ((Integer) mods.read(3)) / 8D);
					Entity disguisedEntity = null;
					DisguiseSound entitySound = null;
					Disguise disguise = null;
					final Entity[] entities = soundLoc.getChunk().getEntities();
					for (final Entity entity : entities) {
						final Disguise entityDisguise = DisguiseAPI.getDisguise(observer, entity);
						if (entityDisguise != null) {
							Location loc = entity.getLocation();
							loc = new Location(observer.getWorld(), ((int) (loc.getX() * 8)) / 8D,
									((int) (loc.getY() * 8)) / 8D, ((int) (loc.getZ() * 8)) / 8D);
							if (loc.equals(soundLoc)) {
								...
								if (entitySound != null) {
									...
									if (entity instanceof LivingEntity)
										try {
											// Use reflection so that this works
											// for either int or double methods
									...
									if (obj == null) {
										...
										final Object nmsEntity = ReflectionManager.getNmsEntity(entity);
										try {
											if (entity instanceof LivingEntity)
												hasInvun = ReflectionManager.getNmsField("Entity", "noDamageTicks")
														.getInt(nmsEntity) == ReflectionManager
																.getNmsField("EntityLiving", "maxNoDamageTicks")
																.getInt(nmsEntity);
											else {
												final Class clazz = ReflectionManager.getNmsClass("DamageSource");
												hasInvun = (Boolean) ReflectionManager
														.getNmsMethod("Entity", "isInvulnerable", clazz)
														.invoke(nmsEntity,
																ReflectionManager.getNmsField(clazz, "GENERIC"));
											}
										} catch (final Exception ex) {
											ex.printStackTrace();
										}
					...
					if (disguise != null)
						if (disguise.isSelfDisguiseSoundsReplaced() || disguisedEntity != event.getPlayer())
							if (disguise.isSoundsReplaced()) {
								String sound = null;
								final DisguiseSound dSound = DisguiseSound.getType(disguise.getType().name());
								if (dSound != null)
								...
								if (sound == null)
									...
									try {
										final int typeId = soundLoc.getWorld().getBlockTypeIdAt(soundLoc.getBlockX(),
												soundLoc.getBlockY() - 1, soundLoc.getBlockZ());
										final Object block = ReflectionManager
												.getNmsMethod("RegistryMaterials", "a", int.class)
												.invoke(ReflectionManager.getNmsField("Block", "REGISTRY").get(null),
														typeId);
										if (block != null) {
											final Object step = ReflectionManager.getNmsField("Block", "stepSound")
													.get(block);
											mods.write(0, ReflectionManager
													.getNmsMethod(step.getClass(), "getStepSound").invoke(step));
										}
									} catch (final Exception ex) {
										ex.printStackTrace();
									}
								// There is no else statement. Because
								// seriously. This should never be null. Unless
								// someone is
								// sending fake sounds. In which case. Why
								// cancel it.
								else {
									mods.write(0, sound);
									...
									if (soundType == SoundType.HURT || soundType == SoundType.DEATH
										...
										// Here I assume its the default pitch
										// as I can't calculate if its real.
										if (disguise instanceof MobDisguise && disguisedEntity instanceof LivingEntity
											...
											if (((MobDisguise) disguise).isAdult() == baby) {
												...
												if (baby) {
													// If the pitch is not the
													// expected
													if (pitch > 97 || pitch < 111)
														return;
													pitch = (new Random().nextFloat() - new Random().nextFloat()) * 0.2F
															+ 1.5F;
													// Min = 1.5
													// Cap = 97.5
													// Max = 1.7
					...
					if ((Byte) mods.read(1) == 2) {
						...
						if (disguise != null && !disguise.getType().isPlayer()
							...
							if (disSound.getSound(soundType) == null
								...
								if (disSound != null) {
									final String sound = disSound.getSound(soundType);
									if (sound != null) {
										final Location loc = entity.getLocation();
										final PacketContainer packet = new PacketContainer(
			...
			public void onPacketSending(PacketEvent event) {
				...
				if (event.getPacket().getIntegers().read(0) == observer.getEntityId())
					if (DisguiseAPI.isSelfDisguised(observer)) {
						// Here I grab the packets to convert them to, So I can
						// display them as if the disguise sent them.
						final PacketContainer[][] transformed = transformPacket(event.getPacket(), observer, observer);
						...
						if (delayedPackets != null && delayedPackets.length > 0)
							Bukkit.getScheduler().scheduleSyncDelayedTask(libsDisguises, () -> {
								try {
									for (final PacketContainer packet : delayedPackets)
										ProtocolLibrary.getProtocolManager().sendServerPacket(observer, packet, false);
								} catch (final InvocationTargetException e) {
									e.printStackTrace();
								}
						...
						if (event.getPacketType() == PacketType.Play.Server.ENTITY_METADATA) {
							event.setPacket(event.getPacket().deepClone());
							for (final WrappedWatchableObject watch : event.getPacket().getWatchableCollectionModifier()
									.read(0))
								if (watch.getIndex() == 0) {
									final byte b = (Byte) watch.getValue();
									byte a = (byte) (b | 1 << 5);
									if ((b & 1 << 3) != 0)
										a = (byte) (a | 1 << 3);
									watch.setValue(a);
							...
							final List<WrappedWatchableObject> watchableList = new ArrayList<>();
							byte b = (byte) 1 << 5;
							...
							final Disguise disguise = DisguiseAPI.getDisguise(event.getPlayer(), event.getPlayer());
		...
		inventoryListener = new PacketAdapter(libsDisguises, ListenerPriority.HIGHEST, PacketType.Play.Server.SET_SLOT,
			...
			public void onPacketReceiving(final PacketEvent event) {
				...
				if (!(event.getPlayer() instanceof com.comphenix.net.sf.cglib.proxy.Factory)
					...
					final Disguise disguise = DisguiseAPI.getDisguise(event.getPlayer(), event.getPlayer());
					// If player is disguised, views self disguises and has a
					// inventory modifier
					if (disguise != null && disguise.isSelfDisguiseVisible()
						...
						if (event.getPacketType() == PacketType.Play.Client.SET_CREATIVE_SLOT) {
							...
							if (slot >= 5 && slot <= 8) {
								if (disguise.isHidingArmorFromSelf()) {
									final int armorSlot = Math.abs((slot - 5) - 3);
									final org.bukkit.inventory.ItemStack item = event.getPlayer().getInventory()
								...
								if (disguise.isHidingHeldItemFromSelf()) {
									final int currentSlot = event.getPlayer().getInventory().getHeldItemSlot();
									if (slot + 36 == currentSlot) {
										final org.bukkit.inventory.ItemStack item = event.getPlayer().getItemInHand();
										if (item != null && item.getType() != Material.AIR) {
											final PacketContainer packet = new PacketContainer(
													PacketType.Play.Server.SET_SLOT);
											final StructureModifier<Object> mods = packet.getModifier();
											mods.write(0, 0);
											mods.write(1, slot);
											mods.write(2, ReflectionManager
													.getNmsItem(new org.bukkit.inventory.ItemStack(0)));
											try {
												ProtocolLibrary.getProtocolManager().sendServerPacket(event.getPlayer(),
														packet, false);
											} catch (final InvocationTargetException e) {
						...
						// If the player switched item, aka he moved from slot 1
						// to slot 2
						else if (event.getPacketType() == PacketType.Play.Client.HELD_ITEM_SLOT) {
							if (disguise.isHidingHeldItemFromSelf()) {
								// From logging, it seems that both bukkit and
								// nms uses the same thing for the slot
								// switching.
								// 0 1 2 3 - 8
								// If the packet is coming, then I need to
								// replace the item they are switching to
								// As for the old item, I need to restore it.
								final org.bukkit.inventory.ItemStack currentlyHeld = event.getPlayer().getItemInHand();
								...
								final org.bukkit.inventory.ItemStack newHeld = event.getPlayer().getInventory()
							...
							final int slot = event.getPacket().getIntegers().read(1);
							...
							if (event.getPacket().getIntegers().read(3) == 1) {
								...
								// Shift clicking is exempted for the item in
								// hand..
							...
							if (clickedItem != null && clickedItem.getType() != Material.AIR)
								...
								if (slot >= 5 && slot <= 8) {
									...
									if (disguise.isHidingHeldItemFromSelf()) {
										final int currentSlot = event.getPlayer().getInventory().getHeldItemSlot();
										// Check if the player is on the same
										// slot as the slot that its setting
			...
			public void onPacketSending(PacketEvent event) {
				...
				if (!(event.getPlayer() instanceof com.comphenix.net.sf.cglib.proxy.Factory)
					...
					final Disguise disguise = DisguiseAPI.getDisguise(event.getPlayer(), event.getPlayer());
					// If the player is disguised, views self disguises and is
					// hiding a item.
					if (disguise != null && disguise.isSelfDisguiseVisible()
							&& (disguise.isHidingArmorFromSelf() || disguise.isHidingHeldItemFromSelf()))
						// If the server is setting the slot
						// Need to set it to air if its in a place it shouldn't
						// be.
						// Things such as picking up a item, spawned in item.
						// Plugin sets the item. etc. Will fire this
						/**
						 * Done
						 */
						if (event.getPacketType() == PacketType.Play.Server.SET_SLOT) {
							...
							if (slot >= 5 && slot <= 8) {
								...
								if (disguise.isHidingHeldItemFromSelf()) {
									final int currentSlot = event.getPlayer().getInventory().getHeldItemSlot();
									// Check if the player is on the same slot
									// as the slot that its setting
									if (slot == currentSlot + 36) {
										final org.bukkit.inventory.ItemStack item = event.getPlayer().getItemInHand();
										if (item != null && item.getType() != Material.AIR) {
											event.setPacket(event.getPacket().shallowClone());
											event.getPacket().getModifier().write(2, ReflectionManager
													.getNmsItem(new org.bukkit.inventory.ItemStack(0)));
										}
									}
								}
							...
							event.setPacket(event.getPacket().deepClone());
							final StructureModifier<ItemStack[]> mods = event.getPacket().getItemArrayModifier();
							final ItemStack[] items = mods.read(0);
							for (int slot = 0; slot < items.length; slot++)
								if (slot >= 5 && slot <= 8) {
									...
									if (disguise.isHidingHeldItemFromSelf()) {
										...
										// slot as the slot that its setting
										if (slot == currentSlot + 36) {
											final org.bukkit.inventory.ItemStack item = event.getPlayer()
													.getItemInHand();
											if (item != null && item.getType() != Material.AIR)
												items[slot] = new org.bukkit.inventory.ItemStack(0);
										}
	...
	public static void setupMainPacketsListener() {
		if (clientInteractEntityListener != null) {
			...
			final List<PacketType> packetsToListen = new ArrayList<>();
			// Add spawn packets
			{
			...
			// Add the packet that makes sure that entities with armor do not
			// send unpickupable armor on death
			...
			mainListener = new PacketAdapter(libsDisguises, ListenerPriority.HIGH, packetsToListen) {
				...
				public void onPacketSending(PacketEvent event) {
					...
					final StructureModifier<Entity> entityModifer = event.getPacket()
							.getEntityModifier(observer.getWorld());
					final org.bukkit.entity.Entity entity = entityModifer
					...
					if (entity == observer)
						return;
					final PacketContainer[][] packets = transformPacket(event.getPacket(), event.getPlayer(), entity);
					if (packets != null) {
						event.setCancelled(true);
						try {
							for (final PacketContainer packet : packets[0])
								ProtocolLibrary.getProtocolManager().sendServerPacket(observer, packet, false);
							final PacketContainer[] delayed = packets[1];
							if (delayed.length > 0)
								Bukkit.getScheduler().scheduleSyncDelayedTask(libsDisguises, () -> {
									try {
										for (final PacketContainer packet : delayed)
											ProtocolLibrary.getProtocolManager().sendServerPacket(observer, packet,
													false);
									} catch (final InvocationTargetException e) {
										e.printStackTrace();
			...
			if (disguise != null) {
				...
				if (sentPacket.getType() == PacketType.Play.Server.UPDATE_ATTRIBUTES) {
					if (disguise.isMiscDisguise())
						...
						final List<WrappedAttribute> attributes = new ArrayList<>();
						for (final WrappedAttribute attribute : sentPacket.getAttributeCollectionModifier().read(0))
				...
				// Else if the packet is sending entity metadata
					...
					if (DisguiseConfig.isMetadataPacketsEnabled()) {
						...
						watchableObjects = DisguiseUtilities.rebuildForVersion(observer, disguise.getWatcher(),
								watchableObjects);
						packets[0] = new PacketContainer(sentPacket.getType());
						final StructureModifier<Object> newMods = packets[0].getModifier();
				...
				// Else if the packet is spawning..
					...
					final PacketContainer[][] spawnPackets = constructSpawnPackets(observer, disguise, entity);
					packets = spawnPackets[0];
					delayedPackets = spawnPackets[1];
				}

				// Else if the disguise is attempting to send players a
				// forbidden packet
				else if (sentPacket.getType() == PacketType.Play.Server.ANIMATION) {
					if (disguise.getType().isMisc() || (packets[0].getIntegers().read(1) == 2
					...
					if (disguise.getType().isMisc())
						packets = new PacketContainer[0];
							...
							&& ((PlayerWatcher) disguise.getWatcher()).isSleeping()) {
						final PacketContainer newPacket = new PacketContainer(PacketType.Play.Server.ANIMATION);
						final StructureModifier<Integer> mods = newPacket.getIntegers();
				...
				// Else if the disguise is moving.
					...
					if (disguise.getType() == DisguiseType.RABBIT
							&& (sentPacket.getType() == PacketType.Play.Server.REL_ENTITY_MOVE
									|| sentPacket.getType() == PacketType.Play.Server.ENTITY_MOVE_LOOK))
						if (entity.getMetadata("LibsRabbitHop").isEmpty()
								|| System.currentTimeMillis()
										- entity.getMetadata("LibsRabbitHop").get(0).asLong() < 100
								|| System.currentTimeMillis()
										- entity.getMetadata("LibsRabbitHop").get(0).asLong() > 500) {
							if (entity.getMetadata("LibsRabbitHop").isEmpty() || System.currentTimeMillis()
									- entity.getMetadata("LibsRabbitHop").get(0).asLong() > 500) {
					...
					if (sentPacket.getType() == PacketType.Play.Server.ENTITY_LOOK
						...
						final StructureModifier<Byte> bytes = packets[0].getBytes();
						final boolean tele = sentPacket.getType() == PacketType.Play.Server.ENTITY_TELEPORT;
						final byte yawValue = bytes.read(tele ? 0 : 3);
						bytes.write(tele ? 0 : 3, getYaw(disguise.getType(), entity.getType(), yawValue));
						final byte pitchValue = bytes.read(tele ? 1 : 4);
						...
						if (tele && disguise.getType() == DisguiseType.ITEM_FRAME) {
							final StructureModifier<Integer> ints = packets[0].getIntegers();
							final Location loc = entity.getLocation();
							final int data = ((((int) loc.getYaw() % 360) + 720 + 45) / 90) % 4;
							if (data % 2 == 0)
								if (data % 2 == 0)
					...
					if (slot < 0)
						slot = 4;
					final org.bukkit.inventory.ItemStack itemstack = disguise.getWatcher().getItemStack(slot);
					if (itemstack != null) {
					...
					if (disguise.getWatcher().isRightClicking() && slot == 4) {
						final ItemStack heldItem = packets[0].getItemModifier().read(0);
						if (heldItem != null && heldItem.getType() != Material.AIR) {
							// Convert the datawatcher
							List<WrappedWatchableObject> list = new ArrayList<>();
							if (DisguiseConfig.isMetadataPacketsEnabled()) {
								list.add(new WrappedWatchableObject(0,
										WrappedDataWatcher.getEntityWatcher(entity).getByte(0)));
								list = disguise.getWatcher().convert(list);
							} else
								for (final WrappedWatchableObject obj : disguise.getWatcher().getWatchableObjects())
									if (obj.getIndex() == 0) {
										list.add(obj);
										break;
									}
							...
							final PacketContainer packetBlock = new PacketContainer(
									PacketType.Play.Server.ENTITY_METADATA);
							packetBlock.getModifier().write(0, entity.getEntityId());
							packetBlock.getWatchableCollectionModifier().write(0, list);
							final PacketContainer packetUnblock = packetBlock.deepClone();
							// Make a packet to send the 'unblock'
							for (final WrappedWatchableObject watcher : packetUnblock.getWatchableCollectionModifier()
									.read(0))
								watcher.setValue((byte) ((Byte) watcher.getValue() & ~(1 << 4)));
							// Send the unblock before the itemstack change so
							// that the 2nd metadata packet works. Why?
							// Scheduler
							// delay.
							packets = new PacketContainer[] { packetUnblock, packets[0], packetBlock };
							// Silly mojang made the right clicking datawatcher
							// value only valid for one use. So I have to reset
							// it.
						}
					}
				}
					...
					if (disguise.getType().isPlayer() && entity.getType() != EntityType.PLAYER) {
						final Location loc = entity.getLocation();
						final byte pitch = getPitch(disguise.getType(), DisguiseType.getType(entity.getType()),
								(byte) (int) (loc.getPitch() * 256.0F / 360.0F));
						final byte yaw = getYaw(disguise.getType(), entity.getType(), sentPacket.getBytes().read(0));
						final PacketContainer rotation = new PacketContainer(
								PacketType.Play.Server.ENTITY_HEAD_ROTATION);
						final StructureModifier<Object> mods = rotation.getModifier();
						mods.write(0, entity.getEntityId());
						mods.write(1, yaw);
						final PacketContainer look = new PacketContainer(PacketType.Play.Server.ENTITY_LOOK);
