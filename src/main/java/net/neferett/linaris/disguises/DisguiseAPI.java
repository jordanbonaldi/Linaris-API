...
public class DisguiseAPI {
	...
	public static Disguise constructDisguise(Entity entity, boolean doEquipment, boolean doSneak, boolean doSprint) {
		final DisguiseType disguiseType = DisguiseType.getType(entity);
		Disguise disguise;
		...
		final FlagWatcher watcher = disguise.getWatcher();
		if (entity instanceof LivingEntity)
			for (final PotionEffect effect : ((LivingEntity) entity).getActivePotionEffects()) {
		...
		for (final Method method : entity.getClass().getMethods())
			if ((doSneak || !method.getName().equals("setSneaking"))
				...
				final int firstCapitalMethod = firstCapital(method.getName());
				if (firstCapitalMethod > 0)
					for (final Method watcherMethod : watcher.getClass().getMethods())
						if (!watcherMethod.getName().startsWith("get") && watcherMethod.getReturnType() == void.class
								&& watcherMethod.getParameterTypes().length == 1) {
							final int firstCapitalWatcher = firstCapital(watcherMethod.getName());
							if (firstCapitalWatcher > 0 && method.getName().substring(firstCapitalMethod)
								...
								if (methodReturn == methodParam)
									try {
										...
										if (value != null) {
											final Class toCast = watcherMethod.getParameterTypes()[0];
	...
	public static void disguiseIgnorePlayers(Entity entity, Disguise disguise, Collection playersToNotSeeDisguise) {
		...
		for (final Object obj : playersToNotSeeDisguise)
	...
	/**
	 * Disguise the next entity to spawn with this disguise. This may not work
	 * however if the entity doesn't actually spawn.
	 *
	 * @param disguise
	 * @return
	 */
	public static int disguiseNextEntity(Disguise disguise) {
		...
		if (disguise.getEntity() != null || DisguiseUtilities.getDisguises().containsValue(disguise))
			disguise = disguise.clone();
		try {
			final Field field = ReflectionManager.getNmsClass("Entity").getDeclaredField("entityCount");
	...
	public static void disguiseToAll(Entity entity, Disguise disguise) {
		if (disguise.getEntity() != null)
			disguise = disguise.clone();
		// You called the disguiseToAll method foolish mortal! Prepare to have
		// your custom settings wiped!!!
		((TargetedDisguise) disguise).setDisguiseTarget(TargetType.SHOW_TO_EVERYONE_BUT_THESE_PLAYERS);
		for (final String observer : ((TargetedDisguise) disguise).getObservers())
	...
	public static void disguiseToPlayers(Entity entity, Disguise disguise, Collection playersToViewDisguise) {
		...
		for (final Object obj : playersToViewDisguise)
			if (obj instanceof String)
				((TargetedDisguise) disguise).addPlayer((String) obj);
			else if (obj instanceof Player)
				((TargetedDisguise) disguise).addPlayer(((Player) obj).getName());
	...
	public static void undisguiseToAll(Entity entity) {
		final Disguise[] disguises = getDisguises(entity);
